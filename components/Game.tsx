...
const Game: React.FC<GameProps> = ({ level, gameState, setGameState, onWin, onLose, images }) => {
  ...
  useEffect(() => {
    const updateLoop = () => {
        ...
        if (hipoBody) {
            ...
            if (loadedImages.current.hipo) ctx.drawImage(loadedImages.current.hipo, -35, -35, 70, 70);
        ...
        level.nests.forEach(nest => {
            for (let i = 0; i < Math.floor(level.beeCount / level.nests.length); i++) {
                piranhas.push(Matter.Bodies.circle(nest.x + (Math.random()-0.5)*40, nest.y + (Math.random()-0.5)*40, 9, { 
                  label: 'piranha', frictionAir: 0.04, restitution: 0.6, density: 0.002, 
  ...
  return (
    <div className="relative">
      <div className="absolute top-4 left-1/2 -translate-x-1/2 w-64 h-6 bg-gray-200 rounded-full border-2 border-white shadow-inner overflow-hidden z-10">
        <div className="h-full bg-gradient-to-r from-cyan-400 to-blue-600 transition-all duration-100" style={{ width: `${(inkRemaining / MAX_INK) * 100}%` }} />
        <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-blue-900 uppercase">Ink Meter</span>
      </div>
